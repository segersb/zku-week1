...
template sudoku() {
    ...
    component puzzleRp[9][9];
    component solutionRp[9][9];

    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            puzzleRp[i][j] = RangeProof(32);
            puzzleRp[i][j].in <== puzzle[i][j];
            puzzleRp[i][j].range[0] <== 0;
            puzzleRp[i][j].range[1] <== 9;

            solutionRp[i][j] = RangeProof(32);
            solutionRp[i][j].in <== solution[i][j];
            solutionRp[i][j].range[0] <== 0;
            solutionRp[i][j].range[1] <== 9;

            assert(puzzleRp[i][j].out == 1); //[assignment] change assert() to use your created RangeProof instead
            assert(solutionRp[i][j].out == 1); //[assignment] change assert() to use your created RangeProof instead
